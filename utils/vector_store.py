...
No chromadb dependency — works on Python 3.14 (Streamlit Cloud).
Functionally equivalent to ChromaDB for this RAG use case.
...
def _load_store(persist_dir: str):
    """Load metadata list and embeddings array from disk."""
...
def _cosine_similarity_batch(query: np.ndarray, matrix: np.ndarray) -> np.ndarray:
    """Cosine similarity between query vector and each row of matrix."""
...
# ---------------------------------------------------------------------------
# SimpleVectorStore — drop-in replacement for ChromaDB collection
# ---------------------------------------------------------------------------
...
class SimpleVectorStore:
    ...
    def upsert(self, ids, embeddings, documents, metadatas):
        ...
        self._meta = new_meta
        self._emb = (np.array(new_emb_list, dtype=np.float32)
                     if new_emb_list else np.empty((0,), dtype=np.float32))
    ...
    def delete(self, ids=None, where=None):
        ...
        self._emb = (self._emb[keep] if len(self._emb) > 0 and keep
    ...
    def get(self, where=None, include=None):
        ...
        return {
            ...
            "metadatas": [{k: v for k, v in r.items() if k not in ("text", "chunk_id")}
                          for r in records],
...
# ---------------------------------------------------------------------------
# Public API — same interface as the original chromadb version
# ---------------------------------------------------------------------------

def get_chroma_client(persist_dir: str) -> str:
    """Returns persist_dir as the 'client' token."""
    return persist_dir


def get_or_create_collection(client_or_dir) -> SimpleVectorStore:
    """Get or create the vector store. Accepts persist_dir string or client token."""
    global _store_cache
    persist_dir = client_or_dir if isinstance(client_or_dir, str) else client_or_dir
...
def add_chunks_to_collection(
    ...
    metadatas = [{"filename": c["filename"], "chunk_index": c["chunk_index"],
                  "upload_time": c["upload_time"]} for c in chunks]
...
def clear_collection(client_or_dir) -> None:
    """Wipe all stored data and reset the cache."""
    global _store_cache
    persist_dir = client_or_dir if isinstance(client_or_dir, str) else client_or_dir
